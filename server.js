...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/api/access") {
    if (enforceRateLimit(req, res, "access")) return;
    let body;
    try {
      body = await readJson(req);
    } catch (error) {
      sendJson(res, error.statusCode || 400, { error: error.statusCode ? error.message : "Invalid access request." });
      return;
    }
