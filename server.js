...
function buildInstructions(context = {}) {
  ...
  const safeContext = {
    ...
    manualContext: truncateText(context.manualContext, 6500),
    messageFormat: {
...
async function handleEmbed(req, res) {
  ...
  try {
    ...
    sendJson(res, error.statusCode || 400, { error: "Could not read the request." });
    return;
  }

  const text = String(payload.text || "").slice(0, 2000).trim();
  if (!text) {
    sendJson(res, 400, { error: "No text to embed." });
    return;
  }
  ...
  try {
    ...
    const data = await apiResponse.json();
    if (!apiResponse.ok) {
      console.error("[OpenAI error]", { timestamp: new Date().toISOString(), action: "embed", message: data?.error?.message });
      sendJson(res, 502, { error: "Pool Pal could not build the search vector." });
      return;
    }

    sendJson(res, 200, { vector: data?.data?.[0]?.embedding || [] });
  } catch (error) {
    console.error("[OpenAI error]", { timestamp: new Date().toISOString(), action: "embed", message: error.message });
    sendJson(res, 502, { error: "Pool Pal could not build the search vector." });
  }
}
...
async function serveFile(req, res) {
  ...
  const isManualFile = publicPath.startsWith("/manuals/") && publicPath.endsWith(".json");
  if (!isAllowed || publicPath.includes("/.") || (!isManualFile && (publicPath.endsWith(".js") || publicPath.endsWith(".json")))) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && req.url === "/api/embed") {
    if (enforceRateLimit(req, res, "embed")) return;
